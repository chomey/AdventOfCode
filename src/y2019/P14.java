...
public class P14 {
		...
		for (int i = 0; i < values.length; ) {
			...
			switch (opcode) {
				...
				case 3:
					...
					values[index] = action(robot);
					i += 2;
					break;
				//Print
				case 4:
					response(robot, firstValue.intValue());
					i += 2;
					break;
				//Jump if true
				case 5:
	...
	private static int action(Robot robot) {
		switch (robot.mode) {
			case MOVING:
				if (robot.targetPath.isEmpty()) {
					//We're at target, so explore it
					for (Direction direction : Direction.values()) {
						Point next = robot.position.apply(direction);
						if (!robot.queued.contains(next)) {
							List<Direction> nextPath = new ArrayList<>(robot.pathTaken);
							nextPath.add(direction);
							robot.nextExplore.add(nextPath);
							robot.queued.add(next);
						}
					}
					if (robot.pathTaken.isEmpty()) {
						robot.targetPath.addAll(robot.nextExplore.poll());
					} else {
						robot.mode = Mode.HOME;
						Direction nextDirection = robot.pathTaken.pollLast().opposite();
						robot.lastDirection = nextDirection;
						return nextDirection.code;
					}
				}
				Direction nextDirection = robot.targetPath.pollFirst();
				robot.lastDirection = nextDirection;
				robot.pathTaken.addLast(nextDirection);
				return nextDirection.code;
			case HOME:
				if (robot.pathTaken.isEmpty()) {
					robot.mode = Mode.MOVING;
					if(robot.nextExplore.size()==0){
						System.out.println("Finito2: " + robot.maxDistance);
						System.exit(0);
					}
					robot.targetPath.addAll(robot.nextExplore.poll());
					Direction targetNextDirection = robot.targetPath.poll();
					robot.lastDirection = targetNextDirection;
					robot.pathTaken.addLast(targetNextDirection);
					return targetNextDirection.code;
				}
				Direction opposite = robot.pathTaken.pollLast().opposite();
				robot.lastDirection = opposite;
				return opposite.code;
		}
		throw new RuntimeException("Shouldn't happen");
	}

	private static void response(Robot robot, int intValue) {
		Point nextPosition = robot.position.apply(robot.lastDirection);
		switch (intValue) {
			case 0:
				//Don't update position
				robot.pathTaken.pollLast();
				robot.mode = Mode.HOME;
				break;
			case 1:
				robot.position = nextPosition;
				if (robot.pathTaken.size() > robot.maxDistance) {
					robot.maxDistance = robot.pathTaken.size();
				}
				break;
			case 2:
//				System.exit(0); //End for part 1
				if (!robot.hasBeenReset) {
					System.out.println("Finito: " + robot.pathTaken.size());
					Point origin = new Point(0, 0);
					robot.position = origin;
					robot.queued.clear();
					robot.queued.add(origin);
					robot.nextExplore.clear();
					robot.pathTaken.clear();
					robot.targetPath.clear();
					robot.maxDistance = 0;
	...
	public static class Robot {
		...
		Deque<Direction> pathTaken = new ArrayDeque<>();
		Deque<Direction> targetPath = new ArrayDeque<>();
		Deque<List<Direction>> nextExplore = new ArrayDeque<>();

		public Robot(Point position, Mode mode) {
			this.position = position;
			this.mode = mode;
			queued.add(new Point(0, 0));
		}
	}
