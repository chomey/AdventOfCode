...
public class P18 {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			boolean pulled = runOn(state, inputArr, x, y);
			if (!pulled) {
				x++;
				continue;
			}
			boolean print = y % 100 == 0;
			if (print) System.out.println("Trying " + y + "...");

			pulled = runOn(state, inputArr, x + 99, y);
			if (!pulled) {
				if (print) System.out.println(String.format("Failing %d,%d", x + 99, y));
				y++;
				x = y/2;
				continue;
			}
			pulled = runOn(state, inputArr, x, y + 99);
			if (!pulled) {
				if (print) System.out.println(String.format("Failing %d,%d", x, y + 99));
				y++;
				x = y/2;
				continue;
			}
			pulled = runOn(state, inputArr, x + 99, y + 99);
			if (!pulled) {
				if (print) System.out.println(String.format("Failing %d,%d", x + 99, y + 99));
				y++;
				x = y/2;
				continue;
			}
